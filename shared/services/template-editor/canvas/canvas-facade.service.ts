...
import { CANVAS_OBJECTS_EXPORT, VariableType } from '../../../consts';
import { CanvasObjectProperties, Layer } from '../../../types';
...
export class CanvasFacadeService {
  ...
  initializeFrame(width: number, height: number, ratioValue?: number): void {
    ...
    this.ratioService.clearStoredDimensions();
  ...
  async generateThumbnailFile(size = 300, fileName?: string): Promise<File> {
    ...
    const name = fileName || `thumbnail-${Date.now()}.png`;
