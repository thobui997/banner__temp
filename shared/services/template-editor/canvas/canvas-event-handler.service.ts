...
import { Canvas, FabricObject, ModifiedEvent, Textbox, TPointerEvent } from 'fabric/*';
import { MoveObjectCommand, RotateObjectCommand, ScaleObjectCommand } from '../../../commands';
...
export class CanvasEventHandlerService {
  ...
  setupEventListeners(canvas: Canvas): void {
    ...
    const objectScalingHandler = (e: any) => {
      ...
      if (this.isButton(obj)) {
        const text = obj.getObjects()[1] as Textbox;
        const scaledWidth = obj.getScaledWidth();
        text.set({
          width: scaledWidth - 20,
          scaleX: 1 / (obj.scaleX || 1),
          scaleY: 1 / (obj.scaleY || 1)
        });
    ...
    const textEditingEnteredHandler = (e: any) => {
      ...
      // Disable snap while editing text
      this.disableSnap();

      if (obj && (obj.type === 'i-text' || obj.type === 'textbox')) {
        this.setupTextSelectionListeners(obj);
      }
    };
    ...
    const textEditingExitedHandler = (e: any) => {
      ...
      // Re-enable snap after editing
      if (!this.snapEnabled) {
        this.enableSnap();
      }

      if (obj && (obj.type === 'i-text' || obj.type === 'textbox')) {
        this.removeTextSelectionListeners(obj);
      }

      // Emit final properties
      this.emitObjectProperties(obj);
    };
    ...
    this.eventHandlers.set('text:editing:entered', textEditingEnteredHandler);
    this.eventHandlers.set('text:editing:exited', textEditingExitedHandler);
  ...
  private isButton(obj: FabricObject): boolean {
    const metadata = obj.get('customMetadata') as any;
    return metadata?.type === VariableType.BUTTON;
  }
  ...
  private setupTextSelectionListeners(textObj: any): void {
    const selectionChangedHandler = () => {
      // Emit current selection styles
      this.emitObjectProperties(textObj);
    };

    // Listen for selection changes
    textObj.on('selection:changed', selectionChangedHandler);

    // Store handler reference for cleanup
    (textObj as any).__selectionHandler = selectionChangedHandler;
  }

  private removeTextSelectionListeners(textObj: any): void {
    const handler = (textObj as any).__selectionHandler;
    if (handler) {
      textObj.off('selection:changed', handler);
      delete (textObj as any).__selectionHandler;
    }
  }
}
